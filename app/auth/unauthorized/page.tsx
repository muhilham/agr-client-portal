import Link from 'next/link'
import Image from 'next/image'
import { AGROASTERY_WA_NUMBER } from '@/lib/payment'
import SignOutButton from './_components/SignOutButton'

export default async function UnauthorizedPage({
  searchParams,
}: {
  searchParams: Promise<{ state?: string; email?: string }>
}) {
  const { state, email } = await searchParams
  const isInactive = state === 'inactive'
  const isUnregistered = state === 'unregistered'
  const waUrl = `https://wa.me/${AGROASTERY_WA_NUMBER}?text=${encodeURIComponent(`Halo Agroastery, saya ingin mengaktifkan akun portal saya. Email yang saya gunakan: ${email ?? ''}`)}`

  return (
    <main className="min-h-screen bg-brand-black flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm flex flex-col items-center gap-8 text-center">
        <Image
          src="/agroastery-logo.svg"
          alt="Agroastery"
          width={180}
          height={28}
        />

        <div className="w-full rounded-xl border border-[rgba(245,235,201,0.25)] bg-brand-midnight p-8 flex flex-col gap-5">
          <div className="text-4xl">🔒</div>
          <h2 className="text-lg font-medium text-brand-crema">Akses Ditolak</h2>

          {isInactive && (
            <p className="text-sm text-brand-parchment leading-relaxed">
              Akun Anda tidak aktif. Hubungi tim Agroastery.
            </p>
          )}

          {isUnregistered && (
            <div className="flex flex-col gap-3 text-sm text-brand-parchment leading-relaxed text-left">
              <p>
                Akun kamu belum terdaftar sebagai klien Agroastery. Hubungi tim kami untuk mengaktifkan akses portal.
              </p>
              {email && (
                <p className="text-xs text-brand-parchment/70">
                  Email yang kamu gunakan saat mendaftar: <span className="text-brand-crema font-medium">{email}</span>
                </p>
              )}
              <p>
                Sertakan email di atas saat menghubungi tim kami agar kami dapat mengaktifkan akun kamu.
              </p>
            </div>
          )}

          {!isInactive && !isUnregistered && (
            <p className="text-sm text-brand-parchment leading-relaxed">
              Akun Anda tidak aktif. Hubungi tim Agroastery.
            </p>
          )}

          {isUnregistered ? (
            <>
              <a
                href={waUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-[#25D366] hover:bg-[#20BD5A] text-white text-sm font-medium px-4 py-2.5 transition-colors"
                data-testid="contact-wa"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                Hubungi via WhatsApp
              </a>
              <SignOutButton />
            </>
          ) : (
            <a
              href={`mailto:hello@agroastery.com`}
              className="text-brand-crema hover:text-brand-honey transition-colors text-sm font-medium"
              data-testid="contact-email"
            >
              hello@agroastery.com
            </a>
          )}
          {(isInactive || (!isInactive && !isUnregistered)) && <SignOutButton />}
        </div>

        <Link
          href="/"
          data-testid="back-to-login-link"
          className="text-sm text-brand-parchment hover:text-brand-crema transition-colors underline underline-offset-4"
        >
          Kembali ke halaman masuk
        </Link>
      </div>
    </main>
  )
}
