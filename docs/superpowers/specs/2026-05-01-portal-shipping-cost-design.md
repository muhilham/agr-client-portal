...
| `lib/biteship.ts` | new | `getBiteshipLocation(id)`, `getBiteshipRates(params)` typed wrappers |
| `lib/shipping.ts` | new | `groupRatesByCourier`, `findRateMatch` pure helpers |
...
export async function getShippingRates(input: { items: { productId: string; quantity: number }[] }): Promise<ShippingRatesResult>
```

## Server Action: `getShippingRates`

1. Auth: load user, look up `clients` row by email. Throw if missing.
2. Default address: `addresses where client_id=client.id and is_default=true` `.maybeSingle()`. None → `{ ok: false, error: 'NO_ADDRESS' }`.
3. Products: re-use `getCatalogForClient(client.id)`. Validate every `productId` in input exists and `quantity >= minQty`. Mismatch → `{ ok: false, error: 'INVALID_CART' }`.
4. Origin: `getBiteshipLocation(process.env.BITESHIP_ORIGIN_LOCATION_ID!)`. Missing env or fetch fail → `{ ok: false, error: 'ORIGIN_NOT_CONFIGURED' }`.
5. Build items: `{ name: product.name, value: product.effectivePrice * quantity, weight: product.ship_weight_grams, quantity }`.
6. Call `getBiteshipRates({...})`. Pass empty `couriers: ''` (return all). Failure → `{ ok: false, error: 'RATES_UNAVAILABLE' }`. Empty `pricing: []` → same.
7. `groupRatesByCourier(rates)` → return `{ ok: true, rates, address: { recipient_name, address_line, postal_code } }`.
...
After existing item validation, before INSERT:
1. Re-run steps 2–6 of `getShippingRates`. Any failure → throw Indonesian message.
2. `findRateMatch(rates, shippingSelection.courier_code, shippingSelection.service_code)`. Null → throw `'Kurir tidak lagi tersedia, silakan pilih ulang'`.
3. INSERT `orders` payload now includes:
   - `shipping_cost: match.price` (Biteship authoritative)
   - `shipping_courier: match.courier_code`
   - `shipping_service: match.courier_service_code`
   - `shipping_etd: match.duration`
4. Telegram payload extended:
   ```typescript
   {
     ...existing,
...
Existing query select extended with `shipping_cost`. Display column shows `total_amount + (shipping_cost ?? 0)`. No new columns added — single grand-total figure.
...
- Origin location not cached. Each checkout = 1 location fetch + 1 rates fetch (review) + 1 location + 1 rates (submit). Acceptable for MVP traffic; revisit if cost concerns.
